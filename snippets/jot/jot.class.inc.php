...
class CJot {
	...
	function Run() {
		...
		$this->config["subject"]["subscribe"] = !is_null($this->Get("subjectSubscribe")) ? $this->Get("subjectSubscribe") : "Новый ответ по интересующей Вас теме";
		$this->config["subject"]["moderate"] = !is_null($this->Get("subjectModerate")) ? $this->Get("subjectModerate") : "Новый ответ на сайте";
		$this->config["subject"]["author"] = !is_null($this->Get("subjectAuthor")) ? $this->Get("subjectAuthor") : "Новый ответ по вашей статье";
		...
		$this->config["validate"] = !is_null($this->Get("validate")) ? $this->Get("validate") : "content:Вы забыли ввести комментарий!";
		...
		if ($this->config["subscription"]["enabled"]) {
				...
				for ($i = 0; $i < $count; $i++) {
						if ($this->config["user"]["id"] != $subscriptions[$i]["userid"] ) {
							...
							mail($user["email"], mb_encode_mimeheader($this->config["subject"]["subscribe"], "UTF-8", "Q"), $message, "From: ".$modx->config['emailsender']."\r\n"."X-Mailer: Content Manager - PHP/".phpversion()."\r\n"."MIME-Version: 1.0"."\r\n"."Content-type: text/plain; charset=".$modx->config['modx_charset']."\r\n");
						}
				}
		}
	}
	...
	function doNotifyAuthor($commentid=0) {
		...
		if ($this->config["moderation"]["notifyAuthor"]) {
			...
			$message = $tpl->Render();
			mb_internal_encoding("UTF-8");
			mail($user["email"], mb_encode_mimeheader($this->config["subject"]["author"], "UTF-8", "Q"), $message, "From: ".$modx->config['emailsender']."\r\n"."X-Mailer: Content Manager - PHP/".phpversion()."\r\n"."MIME-Version: 1.0"."\r\n"."Content-type: text/plain; charset=".$modx->config['modx_charset']."\r\n");
